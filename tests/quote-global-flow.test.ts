...
import { createInitialNativeQuotesPayload, parseNativeQuotesPayload } from "../src/lib/quotes/store";
...
describe("quote global flow", () => {
  it("covers Base V1, quoting, multiline text, V2, variant and duplicate", () => {
    const commercial = applyCommercialMutation(
      ...
      commercialMutationSchema.parse({ action: "create", name: "Agencement accueil", reviewDate: "2026-09-18" }),
      actor,
    ...
    expect(quoting.payload.cases[0]).toMatchObject({ status: "CHIFFRAGE", quoteOwnerName: "Lucien", quoteDueDate: "2026-09-25" });

    const base = applyQuotesMutation(
      createInitialNativeQuotesPayload(),
      quotesMutationSchema.parse({ action: "createDraft", commercialCaseId: caseId, subject: "Agencement accueil", issueDate: "2026-09-16", variantName: "Base", paymentTerms: "45 jours fin de mois" }),
      actor,
      clientId,
      new Date("2026-09-16T08:10:00.000Z"),
    );
    const v1 = base.focusQuoteId;
    expect(base.payload.quotes[0]).toMatchObject({ variantName: "Base", version: 1, status: "DRAFT" });
    expect(base.payload.quotes[0].model).toMatchObject({ clientId, validityDays: 30, paymentTerms: "45 jours fin de mois" });

    const withLine = applyQuotesMutation(
      base.payload,
      quotesMutationSchema.parse({ action: "upsertLine", quoteId: v1, description: "Meuble vasque", unit: "u", quantityInput: "1", unitPriceCents: 125000, saveToLibrary: false }),
      actor,
      undefined,
    ...
    const rich = applyQuoteRichTextUpdate(
      withLine.payload,
      { quoteId: v1, itemId: lineId, text: raw, richText: quoteRichTextFromPlainText(raw) },
      actor,
    ...
    expect(versioned.payload.quotes.find((q) => q.id === v2)).toMatchObject({ variantName: "Base", version: 2, status: "DRAFT" });
    ...
    expect(() => applyQuoteRichTextUpdate(versioned.payload, { quoteId: v1, itemId: lineId, text: "Interdit", richText: quoteRichTextFromPlainText("Interdit") }, actor)).toThrow("QUOTE_NOT_EDITABLE");

    const variant = createQuoteVariant(versioned.payload, v2, actor, new Date("2026-09-16T08:40:00.000Z"));
    expect(variant.payload.quotes.find((q) => q.id === variant.focusQuoteId)).toMatchObject({ variantName: "Variante A", version: 1, commercialCaseId: caseId });
    ...
    expect(copy.payload.quotes.find((q) => q.id === copy.focusQuoteId)).toMatchObject({ variantName: "Copie de Base", version: 1, commercialCaseId: caseId });
    expect(texts(copy.payload, copy.focusQuoteId)).toMatchObject({ plain: expected, rich: expected });
  });
});

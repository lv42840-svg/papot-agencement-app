...
describe("desktop setup encrypted storage", () => {
  it("writes public configuration without plaintext technical secrets", () => {
    ...
    expect(config).not.toHaveProperty("database_url_secret_key");
  });

  it("stores only the Nextcloud secret outside the JSON configuration", () => {
    const userDataPath = makeTempDirectory();
  ...
  it("reopens a complete setup and decrypts the reusable Nextcloud secret", () => {
    const userDataPath = makeTempDirectory();
    setupStore.saveDesktopSetup({
      userDataPath,
      input: validInput(),
  ...
  it("preserves the stable device identifier when setup is saved again", () => {
    const userDataPath = makeTempDirectory();

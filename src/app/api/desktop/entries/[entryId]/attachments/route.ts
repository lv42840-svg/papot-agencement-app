...
import { entriesCapabilities, listSuggestedAssignees, registerEntryAttachments } from "@/lib/entries/mutations";
...
function snapshot(payload: ReturnType<typeof parseEntriesPayload>, owner: { userId: string; displayName: string }, focusEntryId?: string) {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    let mutation = registerEntryAttachments(parseEntriesPayload(opened.resource?.payload), entryId, uploaded, actor);
    ...
    if (saved.status === "conflict") {
      ...
      mutation = registerEntryAttachments(parseEntriesPayload(opened.resource?.payload), entryId, uploaded, actor);
    ...
    return NextResponse.json(snapshot(parseEntriesPayload(saved.resource.payload), desktop.owner, entryId), {
    ...
    return NextResponse.json({ error: code }, { status: statusFor(code), headers: { "Cache-Control": "no-store" } });
  } finally {
    if (ownsLock) {
      void desktop.locks.release({ resource: ENTRIES_RESOURCE, leaseId, owner: desktop.owner }).catch(() => undefined);
    }
  }
}

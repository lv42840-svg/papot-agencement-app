...
export type QuoteWordV2PhotoLoader = (
...
function assertPhotoBytes(photo: QuoteItemPhoto, bytes: Uint8Array): void {
  ...
  if (!matches)
    throw new Error(`QUOTE_WORD_V2_PHOTO_MIME_MISMATCH:${photo.id}`);
}
...
function scaledExtent(
  ...
  const scale = Math.min(
    MAX_IMAGE_WIDTH_EMU / width,
    MAX_IMAGE_HEIGHT_EMU / height,
  );
  return {
...
function annexXml(photos: LoadedPhoto[]): string {
  ...
  photos.forEach((photo, index) => {
    parts.push(
      imageParagraph(photo, 10000 + index),
      captionParagraph(photo.item),
    );
  });
  return parts.join("");
}

function replaceAnchorParagraph(
  documentXml: string,
  replacement: string,
): string {
  ...
  if (
    documentXml.indexOf(ANNEX_ANCHOR, anchorIndex + ANNEX_ANCHOR.length) >= 0
  ) {
...
function ensureContentTypes(xml: string, photos: LoadedPhoto[]): string {
  ...
  for (const [extension, contentType] of needed) {
    ...
    if (closeIndex < 0)
      throw new Error("QUOTE_WORD_V2_CONTENT_TYPES_INVALID");
...
): Array<{ item: QuoteDocumentItem; photo: QuoteItemPhoto }> {
...
export async function renderQuoteWordV2Photos(
  ...
  let relationshipNumber = nextRelationshipId(
    Buffer.from(relsEntry.data).toString("utf8"),
  );
  ...
  const nextRelsXml = appendRelationships(
    Buffer.from(relsEntry.data).toString("utf8"),
    loaded,
  );
  ...
  const nextEntries = entries.map((entry) => {
    if (entry.name === documentEntry.name) {
      return cloneZipEntryWithData(
        entry,
        Buffer.from(nextDocumentXml, "utf8"),
      );
    }
    ...
    if (entry.name === contentTypesEntry.name) {
      return cloneZipEntryWithData(
        entry,
        Buffer.from(nextContentTypes, "utf8"),
      );
    }
    return entry;
  });

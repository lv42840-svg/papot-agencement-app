...
const errorMessages: Record<string, string> = {
  ...
  COMMERCIAL_LOCKED: "Le suivi commercial est modifié sur un autre poste. Réessaie dans quelques secondes.",
  COMMERCIAL_VERSION_CONFLICT: "Le suivi commercial a changé sur un autre poste. Actualise puis réessaie.",
  ...
  COMMERCIAL_REVIEW_DATE_REQUIRED: "Une date de revue ou de relance est obligatoire pour ce statut.",
  COMMERCIAL_CONFIRMATION_DATE_REQUIRED: "La date prévisionnelle de confirmation est obligatoire.",
  COMMERCIAL_QUOTE_OWNER_AND_DATE_REQUIRED: "Le responsable du chiffrage et la date prévue d'envoi sont obligatoires.",
  COMMERCIAL_INSTALL_DATE_REQUIRED: "La date prévisionnelle de pose est obligatoire pour confirmer l'affaire.",
  COMMERCIAL_QUOTE_DATE_NOT_LATER: "La nouvelle date d'envoi doit être postérieure à la date actuelle.",
...
function useCommercialData() {
  ...
  const mutate = useCallback(async (body: MutationBody, successMessage: string) => {
    ...
    try {
      ...
      setError(mutationError instanceof Error ? mutationError.message : "L'action commerciale a échoué.");
      return null;
    } finally {
      setBusy(false);
    }
  ...
  const uploadDocuments = useCallback(
    async (
      ...
      try {
        ...
        setError(uploadError instanceof Error ? uploadError.message : "L'ajout des documents a échoué.");
        return null;
      } finally {
        setBusy(false);
      }
...
export function CommercialWorkspace() {
  ...
  const { snapshot, loading, busy, error, notice, load, mutate, uploadDocuments } = useCommercialData();
  ...
  const activeCount = allCases.filter((item) => !isCommercialClosed(item) && item.status !== "CONFIRMED").length;
  const followUpCount = allCases.filter((item) => commercialNeedsFollowUp(item, now) && !isCommercialClosed(item)).length;
  ...
  return (
    <div className="commercialWorkspace">
      <section className="commercialHeading">
        ...
        <div className="commercialHeadingActions">
          {snapshot?.capabilities.canCreate ? (
            <button type="button" className="primaryButton" onClick={() => setShowCreate((value) => !value)}>
              <Plus size={15} /> Nouvelle affaire
            </button>
          ) : null}
          <button type="button" className="commercialRefresh" onClick={() => void load()} disabled={busy}>
      ...
      <section className="commercialSummary">
        ...
        <SummaryCard label="À relancer" value={followUpCount} tone={followUpCount ? "alert" : undefined} />
        <SummaryCard label="Chiffrages en retard" value={overdueQuoteCount} tone={overdueQuoteCount ? "warning" : undefined} />
        <SummaryCard label="Confirmées" value={confirmedCount} tone={confirmedCount ? "success" : undefined} />
      </section>

      <section className="commercialToolbar">
        <div className="commercialModes">
          <button className={mode === "active" ? "isActive" : ""} type="button" onClick={() => setMode("active")}>Actives</button>
          <button className={mode === "confirmed" ? "isActive" : ""} type="button" onClick={() => setMode("confirmed")}>Confirmées</button>
          <button className={mode === "archives" ? "isActive" : ""} type="button" onClick={() => setMode("archives")}>Archives</button>
        </div>
        <label className="commercialSearch">
          <Search size={14} />
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher affaire, client, lieu, contact…" />
          {query ? <button type="button" title="Effacer" onClick={() => setQuery("")}><X size={13} /></button> : null}
        </label>
      </section>

      {loading && !snapshot ? (
        <div className="commercialLoading"><RefreshCw size={18} className="commercialSpin" /> Chargement du suivi commercial…</div>
      ) : (
        <div className="commercialMasterDetail">
          <aside className="commercialListPanel">
            {search ? <div className="commercialSearchHint">Recherche dans les affaires actives et archivées</div> : null}
            {visibleCases.length === 0 ? (
              <div className="commercialEmptyList"><BriefcaseBusiness size={26} /><strong>Aucune affaire ici</strong></div>
            ) : (
          ...
          <main className="commercialDetailPanel">
            {selected && snapshot ? (
              ...
              <div className="commercialNoSelection"><FolderOpen size={32} /><strong>Sélectionne une affaire</strong></div>
            )}
          </main>
        </div>
      )}
...
function SummaryCard({ label, value, tone }: { label: string; value: number; tone?: "alert" | "warning" | "success" }) {
  ...
  return (
    ...
    >
      <div className="commercialCreateTitle">
        <div><strong>Nouvelle affaire</strong><span>Une vraie piste commerciale. Pour une note à la volée, utilise Entrées.</span></div>
        <button type="button" onClick={onCancel}><X size={15} /></button>
      </div>
      <label><span>Nom de l&apos;affaire *</span><input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="Client, lieu ou nom libre" required /></label>
      ...
      <button type="submit" className="primaryButton" disabled={busy || !name.trim() || !reviewDate}><Plus size={14} /> Créer la piste</button>
    </form>
  );
}

function CommercialListRow({ item, selected, now, onSelect }: { item: CommercialCase; selected: boolean; now: Date; onSelect: () => void }) {
  ...
  return (
    <button type="button" className={`commercialListRow${selected ? " isSelected" : ""}${overdueQuote || followUp ? " needsAction" : ""}`} onClick={onSelect}>
      <div className="commercialListTop">
        <span className={`commercialStatus commercialStatus-${statusTone(item.status)}`}>{COMMERCIAL_STATUS_LABELS[item.status]}</span>
      ...
      <span>{[item.clientName, item.siteLabel].filter(Boolean).join(" · ") || "Client / lieu à compléter"}</span>
      <small>{deadline ? <><CalendarClock size={10} /> {formatDateOnly(deadline)}</> : item.status === "CONFIRMED" ? <><CheckCircle2 size={10} /> Confirmée</> : null}</small>
    </button>
  );
}
      ...
      <header className="commercialDetailHeader">
        <div>
          <div className="commercialListTop">
            <span className={`commercialStatus commercialStatus-${statusTone(item.status)}`}>{COMMERCIAL_STATUS_LABELS[item.status]}</span>
            {overdueQuote ? <span className="commercialAlertPill">Chiffrage en retard</span> : null}
            {followUpDue && !overdueQuote ? <span className="commercialAlertPill">Action commerciale à faire</span> : null}
          </div>
          <h2>{item.name}</h2>
          <p>Créée par {item.createdByName} le {formatDateTime(item.createdAt)}</p>
        </div>
      </header>
...
function DetailsSection({ item, busy, canModify, mutate }: { item: CommercialCase; busy: boolean; canModify: boolean; mutate: (body: MutationBody, successMessage: string) => Promise<CommercialSnapshot | null> }) {
  ...
  return (
    <section className="commercialSection">
      ...
      {canModify ? (
        <button type="button" className="primaryButton commercialFitButton" disabled={busy || !name.trim()} onClick={() => void mutate({ action: "updateDetails", caseId: item.id, name, clientName, siteLabel, contactName, contactPhone, contactEmail, description, nextAction }, "Informations commerciales enregistrées.")}>
...
function StatusSection({
  ...
  const [target, setTarget] = useState<Exclude<CommercialStatus, "LOST" | "ABANDONED">>(item.status === "LOST" || item.status === "ABANDONED" ? "PISTE" : item.status);
  ...
  function saveStatus() {
    void mutate(
      { action: "setStatus", caseId: item.id, status: target, reviewDate: reviewDate || undefined, expectedConfirmationDate: expectedDate || undefined, quoteDueDate: quoteDueDate || undefined, quoteOwnerName: quoteOwnerName || undefined, plannedInstallDate: installDate || undefined },
      "Statut commercial enregistré.",
    );
  }

  const followUpDateNeeded = followUpStatus === "WAITING" || followUpStatus === "PISTE" || followUpStatus === "LIKELY";

  return (
    <section className="commercialSection">
      <div className="commercialSectionTitle"><Clock3 size={15} /> Suivi commercial</div>
      {isQuoteOverdue(item) ? <div className="commercialInlineAlert"><AlertTriangle size={15} /> Le devis devait être envoyé le {formatDateOnly(item.quoteDueDate)}. L&apos;affaire reste en Chiffrage en cours.</div> : null}
      ...
      {canModify ? (
        <div className="commercialStatusEditor">
          ...
          {(target === "PISTE" || target === "WAITING") ? <Field label="Date de revue / retour *"><input type="date" value={reviewDate} onChange={(event) => setReviewDate(event.target.value)} /></Field> : null}
          {target === "LIKELY" ? <Field label="Confirmation attendue *"><input type="date" value={expectedDate} onChange={(event) => setExpectedDate(event.target.value)} /></Field> : null}
          {target === "CHIFFRAGE" ? <><Field label="Responsable chiffrage *"><input list={`quote-owner-${item.id}`} value={quoteOwnerName} onChange={(event) => setQuoteOwnerName(event.target.value)} /><datalist id={`quote-owner-${item.id}`}>{suggestedPeople.map((person) => <option value={person} key={person} />)}</datalist></Field><Field label="Envoi du devis prévu *"><input type="date" value={quoteDueDate} onChange={(event) => setQuoteDueDate(event.target.value)} /></Field></> : null}
          {target === "CONFIRMED" ? <Field label="Date prévisionnelle de pose *"><input type="date" value={installDate} onChange={(event) => setInstallDate(event.target.value)} /></Field> : null}
          <button type="button" className="secondaryButton commercialStatusSave" disabled={busy} onClick={saveStatus}><Save size={13} /> Appliquer</button>
        </div>
      ) : null}
      ...
      {item.status === "CHIFFRAGE" && canModify ? (
        <div className="commercialSubcard">
          ...
          <div className="commercialInlineForm"><Field label="Date de relance obligatoire"><input type="date" value={quoteFollowUpDate} onChange={(event) => setQuoteFollowUpDate(event.target.value)} /></Field><button type="button" className="primaryButton" disabled={busy || !quoteFollowUpDate} onClick={() => void mutate({ action: "markQuoteSent", caseId: item.id, followUpDate: quoteFollowUpDate }, "Devis marqué envoyé. L'affaire est en attente de la relance.")}><Send size={14} /> Marquer envoyé</button></div>
        </div>
      ) : null}
      ...
      {item.status === "CHIFFRAGE" && item.quoteDueDate && canModify ? (
        <details className="commercialDetailsBox">
          ...
          <div className="commercialInlineForm"><Field label="Nouvelle date"><input type="date" value={postponeDate} onChange={(event) => setPostponeDate(event.target.value)} /></Field><Field label="Motif obligatoire"><input value={postponeReason} onChange={(event) => setPostponeReason(event.target.value)} placeholder="Pourquoi le chiffrage est repoussé ?" /></Field><button type="button" className="secondaryButton" disabled={busy || !postponeDate || !postponeReason.trim()} onClick={() => void mutate({ action: "postponeQuoteDue", caseId: item.id, newDate: postponeDate, reason: postponeReason }, "Date prévue du devis repoussée et historisée.")}><CalendarClock size={13} /> Reporter</button></div>
        </details>
      ) : null}
      ...
      {canModify && item.status !== "CONFIRMED" ? (
        <div className="commercialSubcard">
          ...
          <Field label="Compte rendu obligatoire"><textarea rows={2} value={followUpSummary} onChange={(event) => setFollowUpSummary(event.target.value)} placeholder="Résultat, contexte ou suite attendue" /></Field>
          <div className="commercialFollowGrid">
            <Field label="Suite">
              <select value={followUpStatus} onChange={(event) => setFollowUpStatus(event.target.value as CommercialStatus)}>
                <option value="WAITING">En attente</option>
                <option value="PISTE">Piste</option>
                <option value="LIKELY">Ça va tomber</option>
                <option value="CHIFFRAGE">Chiffrage en cours</option>
            ...
            {followUpDateNeeded ? <Field label={followUpStatus === "LIKELY" ? "Confirmation attendue *" : "Prochaine date *"}><input type="date" value={followUpNextDate} onChange={(event) => setFollowUpNextDate(event.target.value)} /></Field> : null}
            {followUpStatus === "CHIFFRAGE" ? <><Field label="Responsable chiffrage *"><input list={`follow-owner-${item.id}`} value={quoteOwnerName} onChange={(event) => setQuoteOwnerName(event.target.value)} /><datalist id={`follow-owner-${item.id}`}>{suggestedPeople.map((person) => <option value={person} key={person} />)}</datalist></Field><Field label="Envoi prévu *"><input type="date" value={quoteDueDate} onChange={(event) => setQuoteDueDate(event.target.value)} /></Field></> : null}
            {followUpStatus === "CONFIRMED" ? <Field label="Date prévisionnelle de pose *"><input type="date" value={installDate} onChange={(event) => setInstallDate(event.target.value)} /></Field> : null}
          </div>
          <button type="button" className="primaryButton commercialFitButton" disabled={busy || !followUpSummary.trim() || (followUpDateNeeded && !followUpNextDate)} onClick={() => void mutate({ action: "recordFollowUp", caseId: item.id, summary: followUpSummary, nextStatus: followUpStatus, nextDate: followUpNextDate || undefined, quoteDueDate: quoteDueDate || undefined, quoteOwnerName: quoteOwnerName || undefined, plannedInstallDate: installDate || undefined }, "Relance enregistrée dans l'historique commercial.")}><Send size={14} /> Enregistrer la relance</button>
        </div>
      ) : null}
    </section>
  );
...
function QuoteNotesSection({ item, busy, canModify, mutate }: { item: CommercialCase; busy: boolean; canModify: boolean; mutate: (body: MutationBody, successMessage: string) => Promise<CommercialSnapshot | null> }) {
...
function DocumentsSection({ item, busy, canModify, uploadDocuments }: { item: CommercialCase; busy: boolean; canModify: boolean; uploadDocuments: ReturnType<typeof useCommercialData>["uploadDocuments"] }) {
  ...
  const grouped = Object.keys(COMMERCIAL_DOCUMENT_CATEGORY_LABELS).map((key) => {
    ...
    return { category: typed, documents: item.documents.filter((document) => document.category === typed) };
  });

  async function submit() {
    const result = await uploadDocuments(item.id, files, { category, versionLabel, variantLabel, isCurrent, isSignedQuote });
    if (result) {
      setFiles([]);
      setVersionLabel("");
      setVariantLabel("");
  ...
  return (
    <section className="commercialSection">
      <div className="commercialSectionTitle"><Paperclip size={15} /> Documents / Chiffrage <span className="commercialCountPill">{item.documents.length}</span></div>
      <p className="commercialHint">Les originaux sont classés automatiquement par affaire et catégorie.</p>

      <div className="commercialDocumentGroups">
        {grouped.map((group) => (
          <div className="commercialDocumentGroup" key={group.category}>
            <div className="commercialDocumentGroupTitle"><strong>{COMMERCIAL_DOCUMENT_CATEGORY_LABELS[group.category]}</strong><span>{group.documents.length}</span></div>
            {group.documents.length === 0 ? <small>Aucun document</small> : group.documents.map((document) => <CommercialDocumentRow key={document.id} item={item} document={document} />)}
          </div>
        ))}
      </div>
      ...
      {canModify ? (
        <div className="commercialUploadBox">
          <div className="commercialUploadFields">
            <Field label="Catégorie"><select value={category} onChange={(event) => { const next = event.target.value as CommercialDocumentCategory; setCategory(next); if (next !== "QUOTE") setIsSignedQuote(false); }}>{Object.entries(COMMERCIAL_DOCUMENT_CATEGORY_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></Field>
            <Field label="Version (facultatif)"><input value={versionLabel} onChange={(event) => setVersionLabel(event.target.value)} placeholder="V2" /></Field>
            <Field label="Variante (facultatif)"><input value={variantLabel} onChange={(event) => setVariantLabel(event.target.value)} placeholder="Variante A" /></Field>
          </div>
          <div className="commercialUploadChecks">
            <label><input type="checkbox" checked={isCurrent} onChange={(event) => setIsCurrent(event.target.checked)} /> Version actuelle</label>
            {category === "QUOTE" ? <label><input type="checkbox" checked={isSignedQuote} onChange={(event) => setIsSignedQuote(event.target.checked)} /> Devis signé</label> : null}
          </div>
          <input ref={inputRef} hidden type="file" multiple disabled={busy} onChange={(event) => { setFiles(Array.from(event.target.files ?? []).slice(0, 12)); event.currentTarget.value = ""; }} />
          <div className="commercialUploadActions">
            <button type="button" className="secondaryButton" disabled={busy} onClick={() => inputRef.current?.click()}><Paperclip size={13} /> Choisir des fichiers</button>
            {files.length > 0 ? <button type="button" className="primaryButton" disabled={busy} onClick={() => void submit()}><Upload size={13} /> Envoyer {files.length}</button> : null}
          </div>
          {files.length > 0 ? <div className="commercialSelectedFiles">{files.map((file, index) => <span key={`${file.name}-${file.size}-${index}`}>{file.name}<button type="button" onClick={() => setFiles((current) => current.filter((_, currentIndex) => currentIndex !== index))}><X size={11} /></button></span>)}</div> : null}
        </div>
      ) : null}
    </section>
  );
...
function CommercialDocumentRow({ item, document }: { item: CommercialCase; document: CommercialDocument }) {
  ...
  return (
    <div className="commercialDocumentRow">
      <span className="commercialDocumentIcon"><Icon size={16} /></span>
      <div className="commercialDocumentMeta">
        <strong>{document.fileName}</strong>
        <span>{formatBytes(document.sizeBytes)} · {document.uploadedByName} · {formatDateTime(document.uploadedAt)}</span>
        <div>{document.isCurrent ? <em>Actuel</em> : null}{document.isSignedQuote ? <em>Signé</em> : null}{document.variantLabel ? <em>{document.variantLabel}</em> : null}{document.versionLabel ? <em>{document.versionLabel}</em> : null}</div>
      </div>
      <div className="commercialDocumentActions">
        ...
        <a className="secondaryButton" href={documentHref(item, document, true)} download={document.fileName}><Download size={13} /> Télécharger</a>
      </div>
      {preview ? <div className="commercialDocumentPreview">{image ? <img src={url} alt={document.fileName} /> : <iframe src={url} title={document.fileName} />}</div> : null}
    </div>
  );
}

function ProvisionSection({ item, busy, canProvision, mutate }: { item: CommercialCase; busy: boolean; canProvision: boolean; mutate: (body: MutationBody, successMessage: string) => Promise<CommercialSnapshot | null> }) {
  ...
  return (
    <section className="commercialSection">
      ...
      <div className="commercialThreeFields">
        ...
        <Field label="Pose (h)"><input type="number" min="0" step="0.5" value={install} onChange={(event) => setInstall(event.target.value)} disabled={!canProvision} /></Field>
      </div>
      {canProvision ? <button type="button" className="secondaryButton commercialFitButton" disabled={busy} onClick={() => void mutate({ action: "updateProvision", caseId: item.id, be: Number(be || 0), workshop: Number(workshop || 0), install: Number(install || 0) }, "Charge potentielle enregistrée.")}><Save size={13} /> Enregistrer la provision</button> : null}
    </section>
  );
}

function CloseSection({ item, busy, canModify, mutate }: { item: CommercialCase; busy: boolean; canModify: boolean; mutate: (body: MutationBody, successMessage: string) => Promise<CommercialSnapshot | null> }) {
  const [reason, setReason] = useState("");
  if (!canModify || item.status === "CONFIRMED") return null;
  return (
    <details className="commercialCloseBox">
      <summary><Archive size={14} /> Clôturer le dossier commercial</summary>
      <p>La clôture retire la charge potentielle mais conserve documents, notes et historique.</p>
      <Field label="Motif facultatif"><textarea rows={2} value={reason} onChange={(event) => setReason(event.target.value)} /></Field>
      <div className="commercialCloseActions">
        <button type="button" disabled={busy} onClick={() => void mutate({ action: "close", caseId: item.id, status: "LOST", reason }, "Affaire classée Perdu.")}>Perdu</button>
        <button type="button" disabled={busy} onClick={() => void mutate({ action: "close", caseId: item.id, status: "ABANDONED", reason }, "Affaire classée Abandonné.")}>Abandonné</button>
      </div>
    </details>
  );
}

function ClosedCommercialCase({ item, busy, canModify, mutate }: { item: CommercialCase; busy: boolean; canModify: boolean; mutate: (body: MutationBody, successMessage: string) => Promise<CommercialSnapshot | null> }) {
  const [reviewDate, setReviewDate] = useState("");
  return (
    <section className="commercialSection commercialArchiveSummary">
      <div className="commercialSectionTitle"><Archive size={15} /> Dossier archivé</div>
      <p>Clôturé le {item.closedAt ? formatDateTime(item.closedAt) : ""}{item.closingReason ? ` · ${item.closingReason}` : ""}</p>
      <p>Les documents, notes et l&apos;historique sont conservés. La charge potentielle n&apos;est pas restaurée automatiquement.</p>
      {canModify ? <div className="commercialInlineForm"><Field label="Nouvelle date de revue *"><input type="date" value={reviewDate} onChange={(event) => setReviewDate(event.target.value)} /></Field><button type="button" className="primaryButton" disabled={busy || !reviewDate} onClick={() => void mutate({ action: "reopen", caseId: item.id, reviewDate }, "Dossier commercial rouvert en Piste.")}><RotateCcw size={14} /> Rouvrir</button></div> : null}
    </section>
  );
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="commercialField"><span>{label}</span>{children}</label>;
}

...
const errorMessages: Record<string, string> = {
  ...
  ENTRIES_LOCKED:
    "Les entrées sont modifiées sur un autre poste. Réessaie dans quelques secondes.",
  ENTRIES_VERSION_CONFLICT:
    "Les entrées ont changé sur un autre poste. La liste a été rechargée.",
  ...
  ENTRY_NOT_DUE_FOR_SNOOZE:
    "Voir plus tard devient disponible après la remontée de l'entrée.",
  SNOOZE_DATE_IN_PAST: "La date Voir plus tard ne peut pas être dans le passé.",
  POSTPONE_DATE_NOT_LATER:
...
function attentionText(entry: EntryRecord): string | null {
  ...
  if (entry.snoozedUntilDate) {
    return `Reportée jusqu'au ${formatDateOnly(entry.snoozedUntilDate)}`;
  }
...
export function EntriesWorkspace() {
  ...
  const mutate = useCallback(
    async (body: MutationBody, successMessage?: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          const base =
            errorMessages[result.error ?? ""] ?? "L'action n'a pas pu être enregistrée.";
          throw new Error(
            result.lockedBy ? `${base} Poste en cours : ${result.lockedBy}.` : base,
          );
        }
        setSnapshot(result);
  ...
  const now = useMemo(
    () => new Date(snapshot?.serverNow ?? Date.now()),
    [snapshot?.serverNow],
  );
  ...
  return (
    <div className="entriesWorkspace">
      <section className="entriesPageHeading">
        <div>
          <h1>Entrées</h1>
          <p>Capture rapide, qualification et suivi des actions.</p>
        </div>
        <button
        ...
        <form className="entriesQuickCapture" onSubmit={submitCapture}>
          <div className="entriesQuickCaptureMain">
            ...
            <textarea
              ...
              placeholder="Nouvelle entrée… Client, lieu, besoin ou note rapide"
              rows={2}
              disabled={busy}
              required
            />
          ...
          <div className="entriesCaptureHint">
            Seul le texte est obligatoire. Les tags et l'urgence restent facultatifs.
          </div>
        </form>
      ) : null}
          ...
          <section className="entriesListPanel">
            <div className="entriesTabs" role="tablist" aria-label="État des entrées">
              <TabButton
                ...
                alertCount={
                  qualifyEntries.filter((entry) => isQualificationAttentionDue(entry, now))
                    .length
                }
    ...
    >
      ...
      <div className="entriesListMeta">
        {entry.status === "TO_QUALIFY" ? (
          ...
          <span>
            <Check size={12} /> Terminée {entry.completedAt ? formatDateTime(entry.completedAt) : ""}
          </span>
        )}
      </div>
    </button>
...
function EntryDetail({
  ...
  const [description, setDescription] = useState(
    entry.structuredDescription ?? entry.rawText,
  );
          ...
          <section className="entriesDetailSection">
            ...
            {capabilities.canQualify ? (
              ...
              <p className="entriesReadOnlyNote">
                Qualification réservée à Nadia et Lucien.
              </p>
            )}
          </section>
  ...
  return (
    <style jsx global>{`
      ...
      .entriesMainGrid {
        min-height: 590px;
        display: grid;
        grid-template-columns: minmax(350px, 0.85fr) minmax(460px, 1.35fr);
        gap: 14px;
      }
      .entriesListPanel,
      .entriesDetailPanel {

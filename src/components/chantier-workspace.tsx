...
function FollowTab({ chantier, busy, capabilities, mutate }: { chantier: ChantierRecord; busy: boolean; capabilities: ChantierCapabilities; mutate: (body: MutationBody, message: string) => Promise<boolean> }) {
  ...
  return (
    <div className="chantierFollowStack">
      ...
      <section className="chantierCard">
        <div className="chantierSectionTitle"><FolderOpen size={15} /> Suivi opérationnel</div>
        <p className="chantierHint">La structure est créée dès le lancement. Les phases ne sont pas un tunnel linéaire : plusieurs espaces peuvent avancer en parallèle.</p>
        <div className="chantierOperationalGrid">
          {CHANTIER_OPERATIONAL_SPACES.map((space) => (
            <div key={space.id}>
              <span>{space.id === "mail" ? <Mail size={15} /> : space.id === "meeting" ? <Users size={15} /> : <FolderOpen size={15} />}</span>
              <strong>{space.label}</strong>
              <small>Structure prête à raccorder</small>
            </div>
          ))}
        </div>
      </section>

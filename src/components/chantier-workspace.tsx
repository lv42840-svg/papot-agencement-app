...
export function ChantierWorkspace({ chantierId }: { chantierId: string }) {
  ...
  const commercialCase = useMemo(
    () =>
      commercialSnapshot?.payload.cases.find(
  ...
  if (loading && !chantiersSnapshot) {
    return (
      <div className="chantierLoading">
        <RefreshCw className="chantierSpin" size={20} /> Chargement du chantier…
      </div>
    );
  }
  ...
  return (
    <div className="chantierWorkspace">
      <section className="chantierHeading">
        <div>
          <Link href="/chantiers" className="chantierBack">
            ← Tous les chantiers
          </Link>
          <div className="chantierTitleRow">
            <span className={`chantierLifecycle chantierLifecycle-${statusTone(chantier)}`}>
              {CHANTIER_STATUS_LABELS[chantier.status]}
            </span>
            {chantier.signedQuoteReminder ? (
              <span className="chantierWarningPill">Devis signé manquant</span>
            ) : null}
          </div>
          <h1>{chantier.name}</h1>
          <p>
            {[chantier.clientName, chantier.siteLabel].filter(Boolean).join(" · ") ||
              "Client / lieu à compléter"}
          </p>
        </div>
        <div className="chantierHeadingActions">
          {commercialCase ? (
            <Link className="secondaryButton" href={`/commercial?focus=${commercialCase.id}`}>
              Ouvrir le commercial
            </Link>
          ) : null}
          <button
            type="button"
            className="chantierRefresh"
            onClick={() => void load()}
            disabled={busy}
          >
...
function LifecycleTab({
  ...
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle">
        <RotateCcw size={17} /> Cycle de vie
      </div>
      <div className="chantierLifecycleSummary">
        <div>
          <span>Statut</span>
          <strong>{CHANTIER_STATUS_LABELS[chantier.status]}</strong>
        </div>
        <div>
          <span>Lancé</span>
          <strong>{formatDateTime(chantier.launchedAt)}</strong>
        </div>
        <div>
          <span>Pose prévue</span>
          <strong>{formatDateOnly(chantier.plannedInstallDate)}</strong>
        </div>
      </div>
        ...
        <div className="chantierLifecycleAction chantierLifecycleSplit">
          ...
          {capabilities.canArchive ? (
            <div>
              ...
              <p>
                Le résumé des réserves, SAV, factures et actions sera enrichi à mesure que ces
                modules sont raccordés. L&apos;archivage reste manuel.
              </p>
              <label className="chantierArchiveCheck">
                <input
                  type="checkbox"
                  checked={archiveReviewed}
                  onChange={(event) => setArchiveReviewed(event.target.checked)}
                />{" "}
                J&apos;ai vérifié les éléments encore ouverts
              </label>
              <button
                type="button"
                className="chantierArchiveButton"
                disabled={busy || !archiveReviewed}
                onClick={() =>
                  void mutate(
                    { action: "archive", chantierId: chantier.id, openItemsReviewed: true },
                    "Chantier archivé.",
                  )
                }
              >
      ...
      {chantier.status === "ARCHIVED" && capabilities.canArchive ? (
        <div className="chantierLifecycleAction">
          ...
          <p>
            Un SAV pourra plus tard être ajouté sans réactiver tout le chantier. Ici, la
            réactivation remet réellement le chantier dans les vues opérationnelles.
          </p>
          <input
            value={unarchiveReason}
            onChange={(event) => setUnarchiveReason(event.target.value)}
            placeholder="Motif obligatoire"
          />
          <button
            type="button"
            className="primaryButton chantierFitButton"
            disabled={busy || !unarchiveReason.trim()}
            onClick={() =>
              void mutate(
                { action: "unarchive", chantierId: chantier.id, reason: unarchiveReason },
                "Chantier réactivé depuis les archives.",
              )
            }
          >
...
function DocumentsTab({
  ...
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle">
        <Paperclip size={17} /> Documents liés{" "}
        <span className="chantierCountPill">{documents.length}</span>
      </div>
      <p className="chantierHint">
        Les documents déjà présents dans l&apos;affaire commerciale restent référencés ici sans
        copie physique. Les nouveaux documents propres au chantier seront raccordés au même système
        documentaire.
      </p>
      {documents.length === 0 ? (
        <div className="chantierDocumentsEmpty">
          <FileText size={28} />
          <strong>Aucun document commercial lié</strong>
        </div>
      ) : (
        <div className="chantierDocuments">
          {documents.map((document) =>
            commercialCase ? (
              <DocumentRow key={document.id} item={commercialCase} document={document} />
            ) : null,
          )}
        </div>
      )}
      <div className="chantierLaunchDocState">
        <div>
          <span>Devis</span>
          <strong>
            {chantier.launchDocuments.quote === "PRESENT"
        ...
        <div>
          ...
          <strong>
            {chantier.launchDocuments.signedQuote === "PRESENT"
              ? "Présent au lancement"
              : "Déclaré absent"}
          </strong>
        </div>
...
function DocumentRow({ item, document }: { item: CommercialCase; document: CommercialDocument }) {
  ...
  return (
    <div className="chantierDocumentRow">
      <span className="chantierDocumentIcon">
        <Icon size={17} />
      </span>
      <div className="chantierDocumentMeta">
      ...
      <div className="chantierDocumentActions">
        ...
        <a className="secondaryButton" href={documentHref(item, document, true)}>
          <Download size={14} /> Télécharger
        </a>
      </div>
      {preview ? (
        <div className="chantierDocumentPreview">
          {image ? (
...
function CapacityTab({
  ...
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle">
        <CalendarClock size={17} /> Charge chantier
      </div>
      <div className="chantierHoursSummary">
        <HourMetric
          label="BE"
          planned={chantier.plannedHours.be}
          actual={chantier.actualHours.be}
          remaining={remaining.be}
        />
        <HourMetric
          label="Atelier"
          planned={chantier.plannedHours.workshop}
          actual={chantier.actualHours.workshop}
          remaining={remaining.workshop}
        />
        <HourMetric
          label="Pose"
          planned={chantier.plannedHours.install}
          actual={chantier.actualHours.install}
          remaining={remaining.install}
        />
      </div>
      <p className="chantierHint">
        Le réalisé est à 0 tant que le module Heures n&apos;est pas raccordé. Lorsqu&apos;il le
        sera, les heures réelles alimenteront automatiquement le reste à faire sans masquer un
      ...
      {canModify ? (
        <div className="chantierHoursEdit">
          <div className="chantierGrid3">
            ...
            <Field label="Atelier prévisionnel (h)">
              <input
                type="number"
                min="0"
                step="0.5"
            ...
            <Field label="Pose prévisionnelle (h)">
              <input
                type="number"
                min="0"
                step="0.5"
          ...
          <Field label="Motif obligatoire de modification">
            <input
              value={reason}
              onChange={(event) => setReason(event.target.value)}
              placeholder="Ex. complément demandé, réévaluation atelier…"
            />
          </Field>
          <button
            type="button"
            ...
            onClick={() =>
              void mutate(
                ...
                "Heures prévisionnelles mises à jour et historisées.",
              )
            }
          >
            <Save size={15} /> Enregistrer la charge
          </button>
        </div>
      ) : null}
    </section>
...
function HourMetric({
  ...
  return (
    <div className={remaining < 0 ? "isNegative" : undefined}>
      <strong>{label}</strong>
      <span>
        <b>{planned}</b> h prévues
      </span>
      <span>{actual} h réelles</span>
    ...
    <section className="chantierCard">
      ...
      <div className="chantierHistory">
        {[...chantier.history].reverse().map((event) => (
          <div key={event.id} className="chantierHistoryRow">
            <span />
            <div>
...
function ChantierStyles() {
  return (
    <style jsx global>{`
      ...
      .chantierHeadingActions {
        display: flex;
        gap: 7px;
      }
      .chantierHeadingActions .secondaryButton,
      .chantierRefresh {
        min-height: 38px;
        padding: 0 12px;
        display: inline-flex;
      ...
      .chantierLifecycle-done {
        background: #eef2fa;
        color: #546e9d;
      }
      .chantierLifecycle-archived {
        background: #ece9ed;
        color: #706a74;
      }
      .chantierWarningPill {
      ...
      .chantierTabs button:hover {
        background: white;
        color: #5f51a1;
      }
      .chantierTabs button.isActive {
        border-color: #a894ec;
        background: white;
        color: #6551c7;
      ...
      .chantierField input:disabled,
      .chantierField textarea:disabled {
        background: #f7f6f8;
        color: #756f7b;
      }
      .chantierFitButton {
      ...
      .chantierLifecycleSummary span {
        color: #8c8693;
        font-size: 11px;
      }
      .chantierLifecycleSummary strong {
        font-size: 13px;
      }
      .chantierLifecycleAction {
        padding: 12px;
        display: grid;
        gap: 9px;
      ...
      .chantierLifecycleAction > p,
      .chantierLifecycleAction div > p {
        margin: 0;
        color: #817b88;
        font-size: 12px;
        line-height: 1.45;
      }
      .chantierLifecycleAction input {
        width: 100%;
        padding: 9px 10px;
        border: 1px solid #ddd9e5;
        border-radius: 7px;
      ...
      .chantierLifecycleSplit > div {
        display: grid;
        gap: 8px;
      }
      .chantierArchiveCheck {
        display: flex;
        align-items: center;
        gap: 7px;
        color: #726b77;
      ...
      .chantierDocumentMeta {
        min-width: 0;
        display: grid;
        gap: 3px;
      }
      .chantierDocumentMeta strong {
        overflow: hidden;
        font-size: 12px;
        text-overflow: ellipsis;
        white-space: nowrap;
      }
      .chantierDocumentMeta small {
        color: #918c99;
        font-size: 10.5px;
      }
      .chantierDocumentActions {
        display: flex;
        gap: 5px;
      }
      .chantierDocumentActions .secondaryButton {
        min-height: 32px;
        padding: 0 8px;
        display: inline-flex;
        align-items: center;
      ...
      .chantierDocumentPreview img {
        display: block;
        max-width: 100%;
        margin: auto;
      }
      .chantierDocumentPreview iframe {
        width: 100%;
        height: 480px;
        border: 0;
      }
      .chantierLaunchDocState {
        display: grid;
        grid-template-columns: repeat(3, minmax(0, 1fr));
        gap: 7px;
      }
      .chantierLaunchDocState > div {
        padding: 10px;
        display: grid;
        gap: 4px;
        border: 1px solid #eeeaf3;
      ...
      .chantierLaunchDocState span {
        color: #8b8592;
        font-size: 11px;
      }
      .chantierLaunchDocState strong {
        font-size: 12.5px;
      }
      .chantierHoursSummary {
        display: grid;
        grid-template-columns: repeat(3, minmax(0, 1fr));
        gap: 8px;
      }
      .chantierHoursSummary > div {
        padding: 12px;
        display: grid;
        gap: 5px;
      ...
      .chantierHoursSummary > div > strong {
        font-size: 13px;
      }
      .chantierHoursSummary span {
        color: #77717e;
        font-size: 11.5px;
      }
      .chantierHoursSummary span b {
        color: #4f4957;
        font-size: 16px;
      }
      .chantierHoursSummary .isNegative {
        border-color: #efc5bc;
        background: #fff6f4;
      }
      .chantierHoursSummary .isNegative span:last-child {
        color: #b34f3d;
        font-weight: 800;
      }
      .chantierHoursEdit {
      ...
      .chantierHistoryRow > div {
        display: grid;
        gap: 3px;
      }
      .chantierHistoryRow strong {
        font-size: 12px;
        font-weight: 650;
      }
      .chantierHistoryRow small {
        color: #98929e;
        font-size: 10.5px;
      }
      .chantierLoading {
      ...
      .chantierSpin {
        animation: chantierSpin 1s linear infinite;
      }
      @keyframes chantierSpin {
        to {
          transform: rotate(360deg);
        }
      }
      .chantierOperationalHeader strong {
        font-size: 15px;
      }
      .chantierOperationalHeader span {
        font-size: 12px;
        line-height: 1.45;
      }
      .chantierOperationalHeader > button {
        font-size: 11.5px;
        min-height: 35px;
      }
      .chantierOperationalCards strong {
        font-size: 13px;
      }
      .chantierOperationalCards small {
        font-size: 11px;
        line-height: 1.4;
      }
      .chantierOperationalCards em {
        font-size: 10.5px;
      }

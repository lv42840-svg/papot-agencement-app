...
export function ChantierWorkspace({ chantierId }: { chantierId: string }) {
  ...
  const tabs: Array<{ id: ChantierTab; label: string; icon: typeof BriefcaseBusiness; badge?: number }> = [
    ...
    { id: "lifecycle", label: "Cycle de vie", icon: RotateCcw },
  ...
  return (
    <div className="chantierWorkspace">
      ...
      <nav className="chantierTabs" aria-label="Rubriques du chantier">
        {tabs.map(({ id, label, icon: Icon, badge }) => (
          <button key={id} type="button" className={activeTab === id ? "isActive" : undefined} onClick={() => setActiveTab(id)}>
            <Icon size={16} /> <span>{label}</span>{badge !== undefined ? <small>{badge}</small> : null}
          </button>
        ))}
      </nav>
      ...
      <section className="chantierTabBody">
        ...
        {activeTab === "lifecycle" ? (
          <LifecycleTab chantier={chantier} busy={busy} capabilities={chantiersSnapshot.capabilities} mutate={mutate} />
        ) : null}
        {activeTab === "follow" ? (
...
function ClientTab({ chantier, busy, canModify, mutate }: { chantier: ChantierRecord; busy: boolean; canModify: boolean; mutate: (body: MutationBody, message: string) => Promise<boolean> }) {
  ...
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle"><BriefcaseBusiness size={17} /> Client / chantier</div>
      ...
      {canModify ? (
        <button type="button" className="primaryButton chantierFitButton" disabled={busy || !name.trim()} onClick={() => void mutate({ action: "updateDetails", chantierId: chantier.id, number, reference, name, clientName, companyName, siteLabel, contactName, contactPhone, contactEmail, description, nextAction }, "Informations du chantier enregistrées.")}><Save size={15} /> Enregistrer</button>
      ) : <p className="chantierHint">Chantier archivé : consultation uniquement tant qu&apos;il n&apos;est pas réactivé.</p>}
    </section>
  );
}

function LifecycleTab({ chantier, busy, capabilities, mutate }: { chantier: ChantierRecord; busy: boolean; capabilities: ChantierCapabilities; mutate: (body: MutationBody, message: string) => Promise<boolean> }) {
  ...
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle"><RotateCcw size={17} /> Cycle de vie</div>
      ...
      {chantier.status === "ACTIVE" && capabilities.canModify ? (
        <div className="chantierLifecycleAction">
          ...
          <button type="button" className="secondaryButton" disabled={busy} onClick={() => void mutate({ action: "markDone", chantierId: chantier.id }, "Chantier passé à Terminé.")}><CheckCircle2 size={15} /> Marquer Terminé</button>
        </div>
      ) : null}
      ...
      {chantier.status === "DONE" && capabilities.canModify ? (
        <div className="chantierLifecycleAction chantierLifecycleSplit">
          <div>
            ...
            <button type="button" className="secondaryButton" disabled={busy} onClick={() => void mutate({ action: "reactivate", chantierId: chantier.id, reason: reactivateReason }, "Chantier remis en Actif.")}><RotateCcw size={15} /> Remettre Actif</button>
          </div>
          {capabilities.canArchive ? (
            <div>
              ...
              <button type="button" className="chantierArchiveButton" disabled={busy || !archiveReviewed} onClick={() => void mutate({ action: "archive", chantierId: chantier.id, openItemsReviewed: true }, "Chantier archivé.")}><Archive size={15} /> Archiver</button>
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      {chantier.status === "ARCHIVED" && capabilities.canArchive ? (
        <div className="chantierLifecycleAction">
          ...
          <button type="button" className="primaryButton chantierFitButton" disabled={busy || !unarchiveReason.trim()} onClick={() => void mutate({ action: "unarchive", chantierId: chantier.id, reason: unarchiveReason }, "Chantier réactivé depuis les archives.")}><RotateCcw size={15} /> Réactiver</button>
        </div>
      ) : null}
    </section>
  );
...
function DocumentsTab({ chantier, commercialCase }: { chantier: ChantierRecord; commercialCase: CommercialCase | null }) {
  ...
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle"><Paperclip size={17} /> Documents liés <span className="chantierCountPill">{documents.length}</span></div>
      ...
      {documents.length === 0 ? (
        <div className="chantierDocumentsEmpty"><FileText size={28} /><strong>Aucun document commercial lié</strong></div>
      ) : (
...
function DocumentRow({ item, document }: { item: CommercialCase; document: CommercialDocument }) {
  ...
  return (
    <div className="chantierDocumentRow">
      <span className="chantierDocumentIcon"><Icon size={17} /></span>
      ...
      <div className="chantierDocumentActions">
        {image || pdf ? <button type="button" className="secondaryButton" onClick={() => setPreview((value) => !value)}><Eye size={14} /> {preview ? "Fermer" : "Aperçu"}</button> : null}
        <a className="secondaryButton" href={documentHref(item, document, true)}><Download size={14} /> Télécharger</a>
      </div>
...
function CapacityTab({ chantier, busy, canModify, mutate }: { chantier: ChantierRecord; busy: boolean; canModify: boolean; mutate: (body: MutationBody, message: string) => Promise<boolean> }) {
  ...
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle"><CalendarClock size={17} /> Charge chantier</div>
      ...
      {canModify ? (
        <div className="chantierHoursEdit">
          ...
          <button type="button" className="primaryButton chantierFitButton" disabled={busy || !reason.trim()} onClick={() => void mutate({ action: "updatePlannedHours", chantierId: chantier.id, be: Number(be || 0), workshop: Number(workshop || 0), install: Number(install || 0), reason }, "Heures prévisionnelles mises à jour et historisées.")}><Save size={15} /> Enregistrer la charge</button>
        </div>
      ) : null}
    </section>
  );
...
function HistoryTab({ chantier }: { chantier: ChantierRecord }) {
  return (
    <section className="chantierCard">
      <div className="chantierSectionTitle"><History size={17} /> Historique</div>
...
function ChantierStyles() {
  return (
    <style jsx global>{`
      .chantierWorkspace{display:grid;gap:14px;width:100%}.chantierHeading{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}.chantierBack{display:inline-block;margin-bottom:7px;color:#7564cc;font-size:12px;font-weight:750;text-decoration:none}.chantierTitleRow{display:flex;gap:6px;align-items:center;flex-wrap:wrap}.chantierHeading h1{margin:6px 0 3px;font-size:28px}.chantierHeading p{margin:0;color:#8b8692;font-size:13px}.chantierHeadingActions{display:flex;gap:7px}.chantierHeadingActions .secondaryButton,.chantierRefresh{min-height:38px;padding:0 12px;display:inline-flex;align-items:center;gap:6px;border:1px solid #ddd9e8;border-radius:8px;background:white;color:#5c5765;font-size:12px;text-decoration:none}.chantierLifecycle,.chantierWarningPill,.chantierCountPill{width:max-content;padding:5px 8px;border-radius:999px;font-size:11px;font-weight:800}.chantierLifecycle-active{background:#e9f7ee;color:#378058}.chantierLifecycle-done{background:#eef2fa;color:#546e9d}.chantierLifecycle-archived{background:#ece9ed;color:#706a74}.chantierWarningPill{background:#fff0dd;color:#a45f18}.chantierMessage{padding:10px 12px;border-radius:9px;font-size:12.5px}.chantierError{border:1px solid #efc4bc;background:#fff5f3;color:#a3493a}.chantierSuccess{border:1px solid #c4e4cf;background:#f1faf4;color:#347850}.chantierTabs{padding:6px;display:flex;gap:5px;overflow-x:auto;border:1px solid #e3deed;border-radius:10px;background:#faf8ff}.chantierTabs button{min-height:40px;padding:0 12px;display:inline-flex;align-items:center;gap:7px;flex:0 0 auto;border:1px solid transparent;border-radius:8px;background:transparent;color:#706978;font-size:12.5px;font-weight:750}.chantierTabs button:hover{background:white;color:#5f51a1}.chantierTabs button.isActive{border-color:#a894ec;background:white;color:#6551c7;box-shadow:0 2px 8px rgb(87 67 150 / .08)}.chantierTabs small{min-width:20px;padding:2px 6px;border-radius:999px;background:#eeeaf6;color:#766c86;font-size:10px}.chantierTabBody{min-height:420px}.chantierCard{padding:15px;display:grid;gap:12px;border:1px solid #e9e5f0;border-radius:11px;background:white}.chantierSectionTitle{display:flex;align-items:center;gap:7px;color:#514c59;font-size:15px;font-weight:800}.chantierCountPill{background:#eeeaf6;color:#6c6478}.chantierGrid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.chantierGrid3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.chantierField{display:grid;gap:5px}.chantierField>span{color:#5e5965;font-size:12px;font-weight:750}.chantierField input,.chantierField textarea{width:100%;padding:10px 11px;border:1px solid #ddd9e5;border-radius:8px;background:white;color:var(--text);outline:none;font:inherit;font-size:13px}.chantierField textarea{resize:vertical}.chantierField input:disabled,.chantierField textarea:disabled{background:#f7f6f8;color:#756f7b}.chantierFitButton{width:max-content}.chantierHint{margin:0;color:#8d8794;font-size:12px;line-height:1.45}.chantierLifecycleSummary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.chantierLifecycleSummary>div{padding:11px 12px;display:grid;gap:4px;border:1px solid #eeeaf3;border-radius:8px;background:#fdfcff}.chantierLifecycleSummary span{color:#8c8693;font-size:11px}.chantierLifecycleSummary strong{font-size:13px}.chantierLifecycleAction{padding:12px;display:grid;gap:9px;border:1px solid #eee9f4;border-radius:9px;background:#fbfaff}.chantierLifecycleAction>p,.chantierLifecycleAction div>p{margin:0;color:#817b88;font-size:12px;line-height:1.45}.chantierLifecycleAction input{width:100%;padding:9px 10px;border:1px solid #ddd9e5;border-radius:7px;background:white;font-size:13px}.chantierLifecycleSplit{grid-template-columns:1fr 1fr}.chantierLifecycleSplit>div{display:grid;gap:8px}.chantierArchiveCheck{display:flex;align-items:center;gap:7px;color:#726b77;font-size:12px}.chantierArchiveButton{width:max-content;min-height:36px;padding:0 11px;display:inline-flex;align-items:center;gap:6px;border:1px solid #e2c5be;border-radius:8px;background:#fff8f6;color:#a05243;font-size:12px;font-weight:750}.chantierDocuments{display:grid;gap:7px}.chantierDocumentsEmpty{min-height:180px;display:grid;place-items:center;align-content:center;gap:7px;color:#938d99}.chantierDocumentsEmpty strong{font-size:13px}.chantierDocumentRow{padding:9px 10px;display:grid;grid-template-columns:34px minmax(0,1fr) auto;gap:9px;align-items:center;border:1px solid #eeeaf3;border-radius:8px;background:#fdfcff}.chantierDocumentIcon{width:32px;height:32px;display:grid;place-items:center;border-radius:7px;background:#eee9ff;color:#6d59c8}.chantierDocumentMeta{min-width:0;display:grid;gap:3px}.chantierDocumentMeta strong{overflow:hidden;font-size:12px;text-overflow:ellipsis;white-space:nowrap}.chantierDocumentMeta small{color:#918c99;font-size:10.5px}.chantierDocumentActions{display:flex;gap:5px}.chantierDocumentActions .secondaryButton{min-height:32px;padding:0 8px;display:inline-flex;align-items:center;gap:4px;font-size:11.5px}.chantierDocumentPreview{grid-column:1/-1;max-height:520px;overflow:auto;border:1px solid #e5e0ec;border-radius:7px;background:#f5f4f7}.chantierDocumentPreview img{display:block;max-width:100%;margin:auto}.chantierDocumentPreview iframe{width:100%;height:480px;border:0}.chantierLaunchDocState{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.chantierLaunchDocState>div{padding:10px;display:grid;gap:4px;border:1px solid #eeeaf3;border-radius:8px}.chantierLaunchDocState span{color:#8b8592;font-size:11px}.chantierLaunchDocState strong{font-size:12.5px}.chantierHoursSummary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.chantierHoursSummary>div{padding:12px;display:grid;gap:5px;border:1px solid #e8e4ee;border-radius:9px;background:#fdfcff}.chantierHoursSummary>div>strong{font-size:13px}.chantierHoursSummary span{color:#77717e;font-size:11.5px}.chantierHoursSummary span b{color:#4f4957;font-size:16px}.chantierHoursSummary .isNegative{border-color:#efc5bc;background:#fff6f4}.chantierHoursSummary .isNegative span:last-child{color:#b34f3d;font-weight:800}.chantierHoursEdit{padding-top:4px;display:grid;gap:9px;border-top:1px solid #eeeaf3}.chantierHistory{display:grid;gap:9px}.chantierHistoryRow{display:grid;grid-template-columns:10px minmax(0,1fr);gap:8px}.chantierHistoryRow>span{width:7px;height:7px;margin-top:6px;border-radius:50%;background:#aa9bdd}.chantierHistoryRow>div{display:grid;gap:3px}.chantierHistoryRow strong{font-size:12px;font-weight:650}.chantierHistoryRow small{color:#98929e;font-size:10.5px}.chantierLoading{min-height:420px;display:grid;place-items:center;align-content:center;gap:8px;border:1px solid #e8e4ef;border-radius:12px;background:white;color:#8a8592;font-size:12px}.chantierLoading strong{font-size:15px}.chantierLoading a{color:#6b58c6}.chantierSpin{animation:chantierSpin 1s linear infinite}@keyframes chantierSpin{to{transform:rotate(360deg)}}
      .chantierOperationalHeader strong{font-size:15px}.chantierOperationalHeader span{font-size:12px;line-height:1.45}.chantierOperationalHeader>button{font-size:11.5px;min-height:35px}.chantierOperationalCards strong{font-size:13px}.chantierOperationalCards small{font-size:11px;line-height:1.4}.chantierOperationalCards em{font-size:10.5px}.chantierOpSpaceTitle strong{font-size:15px}.chantierOpSpaceTitle small{font-size:11px}.chantierOpSpaceTitle>button{font-size:11.5px;min-height:35px}.chantierOpHint{font-size:12px;line-height:1.45}.chantierOpRowMain strong{font-size:13px}.chantierOpRowMain span{font-size:11px}.chantierOpRow select,.chantierInstallRow select,.chantierTechnicalCreate select,.chantierTechnicalCreate input,.chantierInstallRow textarea{font-size:12px}.chantierOpStatus{font-size:10.5px}.chantierOpEmpty strong{font-size:12px}.chantierTechnicalCreateTitle strong{font-size:14px}.chantierTechnicalCreateTitle span{font-size:11px}.chantierTechnicalCreate label>span{font-size:11.5px}.chantierTechnicalCheck{font-size:12px}.chantierTechnicalCreate>p{font-size:11px;line-height:1.4}.chantierTechnicalActions button,.chantierInstallRow>button{font-size:11.5px;min-height:35px}.chantierFutureSpace strong{font-size:14px}.chantierFutureSpace p{font-size:12px;line-height:1.45}.chantierFutureSpace small{font-size:11px}

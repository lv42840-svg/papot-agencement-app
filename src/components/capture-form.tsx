...
export function CaptureForm({
  ...
  const canSend = useMemo(() => title.trim().length > 0 && responsibleUserId.length > 0 && !busy, [title, responsibleUserId, busy]);
    ...
    return (
      <section className="successPanel" aria-live="polite">
        ...
        <div className="buttonRow">
          <button className="primaryButton" type="button" onClick={reset}>Nouvelle capture</button>
          <Link className="secondaryButton" href="/">Retour à l’accueil</Link>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="captureCard">
      <div className="captureHeading">
        ...
        <span className="typeBadge"><Zap size={15} /> Piste commerciale</span>
      </div>

      <div className="captureFields">
        <label className="fullField">Nom libre de la piste
          <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Client, lieu ou sujet…" autoFocus maxLength={240} />
        </label>
        ...
        <fieldset className="priorityField">
          ...
          <div className="segmented">
            <button type="button" className={priority === "NORMAL" ? "active" : ""} onClick={() => setPriority("NORMAL")}>Normale</button>
            <button type="button" className={priority === "URGENT" ? "active urgent" : ""} onClick={() => setPriority("URGENT")}>Urgent</button>
          </div>
        </fieldset>

        <label>À faire pour le <span className="optional">facultatif</span>
      ...
      {tags.length > 0 && (
        <fieldset className="tagField">
          <legend>Tags <span className="optional">facultatif</span></legend>
          <div className="tagList">
            {tags.map((tag) => (
              <button key={tag.id} type="button" aria-pressed={selectedTags.includes(tag.id)} className={selectedTags.includes(tag.id) ? "tag selected" : "tag"} onClick={() => toggleTag(tag.id)}>{tag.label}</button>
            ))}
          </div>
        </fieldset>
      )}
      ...
      <button className="primaryButton sendButton" type="button" disabled={!canSend} onClick={() => void send()}>
